...
public class MyDictApp_ListTest {
	...
	public static void main(String[] args) {
		...
		String filename = "C:\\Users\\Administrator\\Desktop\\ListDict.txt"; // 사용자가 선택할수 있는 내용-> main메소드에서 합니다.
		boolean flag = true;
		try {
		...
		while (flag) {
			...
			case "3":
				try {
					...
					System.out.println("선택한 폴더위치가 잘못되었습니다.");
				}
				break;
			case "4":
	...
	private static void dictRead(String filename) throws FileNotFoundException {
		...
		while (st.hasMoreTokens()) {
			...
			// kor = Arrays.asList(temp.split(", ")); //split 메소드로 분리된 문자열 배열을 리스트 객체로
			// 생성합니다.(고정길이)
	...
	private static void wordAdd() {
		...
		do {
			...
			String eng = sc.nextLine().trim(); // 입력받고 양끝 공백제거
