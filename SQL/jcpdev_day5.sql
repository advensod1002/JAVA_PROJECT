...
SELECT * FROM buy# B, product# P WHERE B.pcode = P.pcode;      
-- ㄴ 기본형식으로 간단 확인
...
SELECT PRICE FROM BUY# B, PRODUCT P WHERE  B.PCODE = P.PCODE
...
-- 아우터 조인 OUTER JOIN : 외부 조인
-- 			정상적으로 조인 조건을 만족하지 않는 행들도 조인결과에 포함
SELECT * FROM BUY# B, CUSTOM# C WHERE B.CUSTOM_ID = C.CUSTOM_ID;
DELETE FROM "BUY#" B WHERE B.CUSTOM_ID = 'momo';
SELECT * FROM BUY# B, CUSTOM# C WHERE B.CUSTOM_ID = C.CUSTOM_ID;
SELECT * FROM BUY# B, CUSTOM# C WHERE B.CUSTOM_ID(+) = C.CUSTOM_ID;

SELECT C.CUSTOM_ID AS ID, NAME, REG_DATE, NVL(PCODE,'구매없음'), NVL(QUANTITY,0)
